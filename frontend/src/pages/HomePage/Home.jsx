...
export default function Home() {
  ...
  const state_neigh = useSelector(
    (state) => state.masterState.neighborhoods_master
  ); // Redux master state
  const loadingState = useSelector(
    (state) => state.masterState.universal_loading_state.payload
  ); // Redux loading state
  ...
  const fetchNeighborhoodAndDateData = async (currentState) => {
    ...
    if (
      currentState.currentNeigh === "boston_city" ||
      currentState.currentNeigh === undefined ||
      currentState.currentNeigh === "" ||
      currentState.hasOwnProperty("CurrentTrack")
    ) {
    ...
    const data = await DataMethods.getgetNeighborhoodAndDateData(
      dates[0],
      dates[1],
      currentState.currentNeigh
    );
