...
export const resolvers = {
	Mutation: {
		addRssFeed: async (_, { url, userID }, context) => {
			...
			const result = await rss_data.findOneAndUpdate(
				filter,
				update,
				options,
			);
	...
	Query: {
		...
		getRssLinkByUserId: async (_, args, context) => {
			const { db } = context;
			const rss_data = db.collection("rss_links");
			const queryResult = rss_data
				.find({ userID: args.user_id })
				.toArray();
			return queryResult;
		},
		// CSV Upload Resolver
		...
		demographicsByTracts: async (
			...
			const { db } = context;
			const tracts_data: Collection<ITracts> =
				db.collection("tracts_data");
		...
		orgArticlesPastWeek: async (_, args, context): Promise<IArticles[]> => {
			const { db } = context;
			const articles_data = db.collection("articles_data");
		...
		getArticlesByOrg: async (_, args, context): Promise<IArticles[]> => {
			const { db } = context;
			const article_data: Collection<IArticles> =
				db.collection("articles_data");
			const articles: IArticles[] = await article_data
