...
const queryResolver = {
  ...
  queryDateAndNeighborhood: async ({dateFrom, dateTo, neighborhood}) => { // Remember to use the deconstructor operator
    ...
    try {  
      ...
      const queryResult = await Promise.all([dateCursor.toArray(), neighborhoodCursor.toArray()]).then( async(_res) => {
        ...
        let ArrayIntersectArticles = Array.from(new Set([...SetOfArticleWithAssociatedTracts].filter((x) => SetOfArticleDataTract.has(x))));
  ...
  queryCensusData: async ({dateFrom, dateTo, censusTract}) => {
    await client.connect();
    infoLogger("[queryCensusData]", `Querying Pipeline with parameters: dateFrom: ${dateFrom}, dateTo: ${dateTo}, censusTract: ${censusTract}`);
    ...
    try {
      ...
      const queryResult = await Promise.all([dateCursor.toArray(), tactCursor.toArray()]).then( async(_res) => {
        if (_res[0].length == 0  || _res[1].length == 0) { // Will write a better guard
          warningLogger(
            "[queryCensusData]", 
            `One of the Query rendered empty. 
